  ...
  return (
    <div className="h-full w-80 border-l border-[var(--color-border)] bg-[var(--color-bg-secondary)] flex flex-col">
      <div className="h-12 border-b border-[var(--color-border)] flex items-center justify-between px-4">
        <h3 className="text-sm font-semibold text-[var(--color-text-primary)]">AI Context</h3>
        ...
        {contextItems.length === 0 ? (
          ...
          <div className="space-y-2">
            {contextItems.map((item) => (
              <div key={item.id} className="p-2 rounded bg-[var(--color-bg-primary)] border border-[var(--color-border)]">
                <div className="flex items-center gap-2">
                  <span className="text-xs">{item.type === 'note' ? '📝' : item.type === 'file' ? '📄' : '💬'}</span>
                  <span className="text-sm font-medium text-[var(--color-text-primary)]">{item.title}</span>
                </div>
                <div className="text-xs text-[var(--color-text-secondary)] mt-1">{item.path}</div>
              </div>
            ))}
          </div>
        )}
