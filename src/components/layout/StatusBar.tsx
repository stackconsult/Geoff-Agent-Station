...
import { GitBranch, Check, AlertCircle, RefreshCw, FolderOpen, Settings, GitMerge } from 'lucide-react'
...
const SYNC_CONFIG: Record<SyncStatus, { label: string; colorClass: string; spin?: boolean }> = {
  idle:          { label: 'Synced',         colorClass: 'text-[var(--color-success)]' },
  syncing:       { label: 'Syncing…',       colorClass: 'text-[var(--color-accent-primary)]', spin: true },
  error:         { label: 'Sync error',     colorClass: 'text-[var(--color-error)]' },
  conflict:      { label: 'Conflict',       colorClass: 'text-[var(--color-warning)]' },
  pull_required: { label: 'Pull required',  colorClass: 'text-[var(--color-accent-secondary)]' },
}
...
export function StatusBar({
  ...
  const SyncIcon = syncStatus === 'syncing' || isVaultReloading
    ? RefreshCw
    : syncStatus === 'error'
    ? AlertCircle
    : syncStatus === 'conflict'
  ...
  return (
    <div className={cn(
      ...
      <div className="flex items-center gap-3 text-[var(--color-text-muted)]">
        ...
        {modifiedCount > 0 && (
          <span className="text-[var(--color-warning)]">{modifiedCount} modified</span>
        )}
        {branch && (
      ...
      <Button
        ...
        onClick={onSync}
      >
        <SyncIcon className={cn('h-3 w-3', (cfg.spin || isVaultReloading) && 'animate-spin')} />
        <span>{cfg.label}</span>
        {lastSyncTime && syncStatus === 'idle' && (
          <span className="text-[var(--color-text-muted)]">{formatLastSync(lastSyncTime)}</span>
        )}
      </Button>
